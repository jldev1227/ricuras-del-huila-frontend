...
import { Bell, ChevronRight, LogOut, MapPin, Menu, X } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const [isLoadingSucursal, setIsLoadingSucursal] = useState(true);
  ...
  // Validar primero el auth-storage
  useEffect(() => {
    const authStorage = localStorage.getItem("auth-storage");
    if (!authStorage) {
      router.push("/auth/login");
  ...
  }, [isLoadingSucursal, sucursalActual, router]);

  // Mostrar loader mientras verifica o si no hay sucursal (durante la redirección)
  if (isLoadingSucursal || !sucursalActual) {
    return null; // No mostrar nada, solo redirigir
  }

  // Si hay sucursal, mostrar el layout normal del POS
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-auto flex flex-col">
        ...
        <header className="bg-white border-b border-gray-200 h-16 lg:h-20 flex items-center justify-between px-4 lg:px-6 flex-shrink-0 sticky top-0 z-30">
          ...
          {/* Logo mobile */}
          <img src="/logo.png" alt="Logo" className="w-24 h-24 lg:hidden" />
